...
def main():
    ...
    for _, batch in dataloader_iterator:
        with torch.inference_mode(mode=True):
            output = verifier.scoring_sequences_by_step(
                batch["input_ids"], agg_mode=inference_args.agg_mode
            )  # (batch_size,)

            if inference_args.agg_mode == "full":
                # (batch_size, seq_len)
                # => (batch_size, n_total_seq, 1)
                assert output.shape[1] == batch["input_ids"].shape[1], (
                    f"seq_len ({output.shape[1]}) and n_total_seq ({batch['input_ids'].shape[1]}) must be consistent!"
                )
                v_scores = output.unsqueeze(2)  # (batch_size, seq_len, 1)
            else:
                # 'min', 'max', 'mean', 'last'
                n_total_seq = batch["input_ids"].shape[1]
                v_scores = (
                    output.unsqueeze(1).unsqueeze(2).expand(-1, n_total_seq, 1)
                )  # (batch_size, n_total_seq, 1)
    ...
    if accelerator.num_processes != 1:
        all_idxs1_gather, all_idxs2_gather, all_vscores_gather = tuple(
        ...
        all_idxs1_gather, all_idxs2_gather, all_vscores_gather = (
            all_idxs1_list,
            all_idxs2_list,
            all_vscores_list,
        )
    ...
    # record
    for idx1, idx2, sol_vscores in zip(
        all_idxs1_gather, all_idxs2_gather, all_vscores_gather
    ):
