...
class DataArguments:
    ...
    data_dir: str = field(
        default="data/gsm8k/", metadata={"help": "Path to the training data."}
    )
    target_set: str = field(default="train")
    loss_on_prefix: bool = field(
        default=False, metadata={"help": "Whether to compute loss on the prefix"}
    )
...
def main():
    ...
    model_args, data_args, training_args, generation_args, output_args = (
    ...
    # config tokenizer，special tokens
        ...
        for local_step, batch in train_dataloader_iterator:
            ...
            if accelerator.is_main_process:
                train_dataloader_iterator.set_postfix(
                    epoch=cur_epoch, step=local_step, loss=loss.item()
                )
