...
from utils.math500.decoding import INVALID_ANS, extract_answers, get_answer_label
...
class GeneratorAnswerAcc:
    ...
    def __call__(self, completions: List[str], gts: List[str]):
        ...
        corrs = [get_answer_label(answer, gt) for answer, gt in zip(answers, gts)]
...
class MultiSamplingAnswerAcc:
    ...
    def end_the_sol_epoch(self):
        if self.world_size != 1:
            gathered_answers, gathered_gts = tuple(
                [None] * self.world_size for _ in range(2)
            )
            for obj, container in [
            ...
            for answers_gpus, gts_gpus in zip(
                zip(*gathered_answers), zip(*gathered_gts)
            ):
    ...
    def get_metric(self, n_solution: int = 3, reset=True):
        ...
        # answers.shape = [n_solution, n_question] -> [n_question, n_solution]
        ...
        # majority vote
        acc_majority = np.mean(
            [is_majority(a, gt, ignore=INVALID_ANS) for a, gt in zip(answers, gts)]
        )
