...
import transformers
from utils.constants import DEFAULT_PAD_TOKEN, DEFAULT_UNK_TOKEN
...
def build_model(model_args: dataclass, training_args: dataclass):
    ...
    # Step 3: Align special token ids between tokenizer and model.config
...
def load_model_for_training(model_args: dataclass, training_args: dataclass):
    # Step 1: Initialize LLM
    print(f"+ [Model] Initializing LM: {model_args.model_name_or_path}")
    model = transformers.AutoModelForCausalLM.from_pretrained(
        model_args.model_name_or_path,
...
def save_training_args_with_accelerator(accelerator: Accelerator,
    ...
    output_file = os.path.join(output_dir, 'training_args.json')
